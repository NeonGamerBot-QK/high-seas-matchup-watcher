...
await page.evaluate(() => {
  //@ts-ignore
  document.getElementsByClassName('bg-white text-black p-2 px-3 sm:px-6 w-fit rounded-lg text-base linkPop')[0].click()
})
...
const code:string = await new Promise(r=>process.stdin.once('data', d=>r(d.toString().trim())))
...
for(const project of [json.project1, json.project2]) {
  ...
  if(await db.project.findFirst({where: {hs_id: project.id}})) continue;
  fp++;
